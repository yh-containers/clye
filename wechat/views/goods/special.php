...
<script>
    var url = '<?=\yii\helpers\Url::to(['show-list','cid'=>$cid,'is_special'=>1])?>';
                ...
                ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
                    ...
                    $.get(url+(url.indexOf('?')===-1?'?':'&')+'page='+page, function(res){
                        ...
                        layui.each(res.data, function(index, item){
                            var info = item.hasOwnProperty('info')?item.info:[];
                            var html ='<li class="good_item">\n' +
                                '                    <a href="'+detail_url+(detail_url.indexOf('?')===-1?'?':'&')+'id='+item.id+'">\n' +
                                '                        <div class="img">\n' +
                                '                            <img class="lazyload" src="'+item.cover_img+'" alt="">\n' +
                                '                        </div>\n' +
                                '                        <div class="prolist_info">\n' +
                                '                            <div class="name">'+item.name+'</div>\n'+
                                '                            <div class="desc">'+item.intro+'</div>\n'+
                                '                            <div class="number">批号优于:'+item.p_no+'</div>\n';
